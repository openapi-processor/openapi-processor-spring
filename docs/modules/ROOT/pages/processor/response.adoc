...
. Another case is WebFlux, where we need the result to be either a `Flux<>` in case of an array type
or a `Mono<>` in case it is not an array type. This is possible using the *single* mapping.
